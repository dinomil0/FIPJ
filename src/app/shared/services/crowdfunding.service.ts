...
export class CrowdfundingService {
  ...
  getSelectedUserCrowdListing(email: string): Observable<any> {
    return new Observable((observer) => {
      this.crowdfundRef.where("userEmail", "==", email).onSnapshot((querySnapshot) => {
        let crowdfundingListing = [];
        querySnapshot.forEach((doc) => {
          let data = doc.data();
          let p = new Crowdfunding(data.username, data.nameProduct, data.goalAmt, data.receiveAmt,
  ...
  listingUpdate(c: Crowdfunding, updateName: string, updateEcoRating: number, updateDescription: string) {
    const ref = this.crowdfundRef.doc(c.id);
    // Update compulsory fields. Do not update id and image
    ref.update({
      username: c.username,
      nameProduct: updateName,
      goalAmt: c.goalAmt,
      receiveAmt: c.receiveAmt,
      description: updateDescription,
      ecoRating: updateEcoRating,
      userEmail: c.userEmail,
      image: c.image,
      imagePath: c.imagePath,
      status: c.status
